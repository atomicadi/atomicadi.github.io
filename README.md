...
_ item 1 **FORT (Fortran Operator for Rate Theory):** A generalized FORTRAN code (with LAPACK routines) that evaluates partition functions for reaction
...
_ item 2 **Microcanonical normal-mode sampling** is implemented in Fortran to generate initial conditions at fixed total energy (NVE) for molecular dynamics/trajectory simulations. [Link](https://github.com/atomicadi/Microcanonical-normal-mode-sampling_in-Fortran)

_ item 3 **Fortran 1D Fourier Grid Hamiltonian (FGH)** solver to compute bound- state spectra for the Morse potential of H<sub>2</sub>, and the double-well potential of NH<sub>3</sub>. [Link](https://github.com/atomicadi/Fourier-Grid-Hamiltonian_in-Fortran) 
